# -*- coding: utf-8 -*-
"""
main_note.py - Phiên bản ghi chú giải thích bản chất (Cái này để làm gì? Thông số nghĩa là gì?)
Sử dụng: python main_note.py [--mode webcam|image|video]
"""

import os, sys, time, argparse, numpy as np, cv2, torch  # [Dòng 6] Nạp các công cụ bổ trợ (đo thời gian học, mở webcam, vẽ khung mặt, chạy AI). Không có thư viện này máy tính sẽ không thể xử lý hình ảnh.
...
from datetime import datetime  # [Dòng 8] Lấy ngày giờ thực tế hiện tại để đặt tên ảnh chụp màn hình khi người dùng bấm nút chụp ảnh (phím 's').
from hsemotion.facial_emotions import HSEmotionRecognizer  # [Dòng 9] Nạp mô hình AI EfficientNet-B0 pretrained (đã học sẵn từ 450.000 khuôn mặt trên thế giới).
...
DISPLAY = {'Anger':'Angry','Happiness':'Happy','Sadness':'Sad','Contempt':'Disgust'}  # [Dòng 16] Đổi tên nhãn sang nhãn ngắn gọn, thân thiện để hiển thị đẹp mắt trên màn hình camera.
...
# ── Emotion Engine (Bộ máy xử lý cảm xúc trung tâm) ──
class EmotionEngine:  # [Dòng 20] Tạo lớp đối tượng xử lý: Quét tìm khuôn mặt, tiền xử lý CLAHE, đoán cảm xúc bằng AI và bám vết người học.
    def __init__(self, model_name='enet_b0_8_best_afew'):  # [Dòng 21] Hàm chạy các cài đặt ban đầu khi khởi động bộ máy EmotionEngine.
        # Override torch.load to bypass weights_only warning in HSEmotionRecognizer  # [Dòng 22] Tắt cảnh báo PyTorch
        torch_load = torch.load  # [Dòng 23] Lưu tạm hàm nạp model gốc của PyTorch.
        torch.load = lambda f,**kw: torch_load(f,**{**kw,'weights_only':False})  # [Dòng 24] Ghi đè hàm load để ép weights_only=False, tránh các dòng cảnh báo bảo mật màu đỏ làm bẩn màn hình.
    ...
    def detect(self, frame):  # [Dòng 34] Hàm tìm tọa độ các khuôn mặt trên khung hình camera.
    ...
    def predict(self, face_img):  # [Dòng 38] Hàm tính toán cảm xúc cho một khuôn mặt đã cắt ra.
        gray = cv2.cvtColor(face_img, cv2.COLOR_BGR2GRAY)  # [Dòng 39] Chuyển khuôn mặt đã cắt sang ảnh xám Grayscale để chuẩn bị cân bằng sáng.
        ...
        # Anh xa 8 lop cua HSEmotion sang 6 lop cua bai toan  # [Dòng 43] Chu thích remapping
    ...
    def process(self, frame):  # [Dòng 48] Hàm tổng xử lý chính trên khung hình camera.
        ...
        if len(faces) > 1:  # [Dòng 51] Nếu camera quét thấy nhiều hơn 1 người học.
            faces = sorted(faces, key=lambda f: f[2]*f[3], reverse=True)  # [Dòng 52] Sắp xếp khuôn mặt to nhất lên đầu để ưu tiên học sinh chính ngồi trực diện camera, bỏ qua người đi lại phía sau.
        ...
        for (x,y,w,h) in faces:  # [Dòng 54] Vòng lặp duyệt qua từng khuôn mặt phát hiện được.
            if w>=48 and h>=48:  # [Dòng 55] Bỏ qua các khuôn mặt quá nhỏ hoặc ở quá xa camera (yêu cầu kích thước tối thiểu 48x48 pixel).
                k = ((x+w//2)//self.GRID*self.GRID, (y+h//2)//self.GRID*self.GRID)  # [Dòng 56] Nhóm tọa độ tâm mặt về lưới 60px làm ID bám vết khuôn mặt để lấy đúng bộ đệm lịch sử.
        ...
        for (x,y,w,h) in faces:  # [Dòng 60] Vòng lặp thứ hai để tiến hành đoán cảm xúc cho từng người học.
            ...
            self.bufs[k].append(scores)  # [Dòng 66] Đẩy kết quả phần trăm mới vào hàng đợi bộ nhớ.
            avg = np.mean(self.bufs[k], axis=0)  # [Dòng 67] Lấy trung bình cộng xác suất của 10 khung hình để nhãn hiển thị mượt mà, không bị nhấp nháy liên tục khi học sinh chớp mắt.
            idx = np.argmax(avg)  # [Dòng 68] Tìm chỉ số index của cảm xúc có điểm số trung bình lớn nhất.
...
# ── Session Stats (Thống kê và xuất Dashboard) ──
class SessionStats:  # [Dòng 79] Tạo đối tượng ghi nhận lịch sử biểu cảm và vẽ biểu đồ báo cáo khi đóng ứng dụng.
    ...
    def report(self, out_dir='reports'):  # [Dòng 89] Hàm tự động vẽ và xuất Dashboard reports/emotion_report.png khi buổi học kết thúc.
        ...
        emo2y = {e:i for i,e in enumerate(self.order)}  # [Dòng 99] Ánh xạ tên cảm xúc sang tọa độ số nguyên 0-5 làm mốc vẽ đồ thị.
        ...
        print(f'  Da luu: reports/emotion_report.png')  # [Dòng 117] In thông báo đã ghi file Dashboard ra màn hình đen console.

# ── Draw Helpers (Vẽ khung đồ họa giao diện camera) ──
...
def mode_video(args):  # [Dòng 160] Hàm chạy chế độ đọc tệp video offline có sẵn.
    ...
    try:  # [Dòng 172] Khối try an toàn để đọc tệp video.
        while True:  # [Dòng 173] Vòng lặp vô hạn đọc từng khung hình video tuần tự.
            ...
            if not ret: break  # [Dòng 175] Khi video đã chạy hết (het khung hinh), thoát khỏi vòng lặp.
